...
%
% INPUT
% - TABLE_INDEX (integer) -     Used for results reading into matlab for purpose of report generation, 
%                               as well as results comparison, when dealing with large number of
%                               simulations - see main_sample_run for an example of such a "run all combinations" script
% - RUNNING_FUNCTION_NAME (string) - Used for simulation and report purposes. Highly useful when dealing with large number of simulations. 
%                               All reports and results are stored with a folder with this name.
% - NUM_CPUS (integer) -        Number of cpus to use. If larger than 1, parallel for loops are used for scattering 
%                               of the images, and for model training for different parameters. Parallel loops are not used for 
%                               the dimensionality reduction, because they (mostly PLS) require large amounts of memory.
% - FEATURE_TRANSFORM_TYPE (string) - string containing numbers ('0',..'9') where each number corresponds to a 
%                               different dimensionality reduction technique. The order of the numbers in string is highly relevant. 
%                               Example: '072' - in this case, we first execute '0', which stands for elimination of all FEATURES which are equal
%                               to 0 across all different samples. After that '7' executes class PLS, and in the end '2' normalizes features to 
%                               range [0,1]. For the list of all supported normalization and dimensionality reduction techniques and
%                               their numbers take a look below in the code.
% - NUM_FEATURES_FINAL (integer) - desired final number of features after the dimensionality reduction. 
%                               Caution: for PLS this number is devided by 10 for each class, resulting in NUM_FEATURES_FINAL/10 features selected, 
%                               but in the end we again get NUM_FEATURES_FINAL, which is the number of features fed into the SVM classifier.
% - SIDE_CUT (integer) -        Obsolete, to be removed. At one point used to cut side pixels of an image, e.g., 32 -> 28
% - SET_TYPE (integer 1,2 or 3) - Take a look in Dataset_MNIST for detailed description. 
%                               Default 1 - select samples from the begining of MNIST dataset such that it is balanced. (when possible)
% - NUM_TRAIN_SAMPLES (integer <= 60000)
% - NUM_TEST_SAMPLES (integer <= 10000)
% - VARARGIN                    - Feature transform specifications to use (possible to specify different one for each layer), and svm parameters.
%                               See sample_multiple_run or sample_single_run for sample calls of this main script.
%
...
% Read arguments from varargin
[num_layers, tsfms, features_variance_threshold,c_start,c_step,c_stop,g_start,g_step,g_stop,filename] = TransformInputArguments(varargin{1:length(varargin)});
